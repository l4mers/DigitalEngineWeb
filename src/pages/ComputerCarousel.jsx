...
const ComputerCarousel = ({ venueData }) => {
  const validImages = venueData.venueMedia.filter((item) => item.image !== null);
  const remainingImages = validImages.length - 4;
  const [open, setOpen] = useState(false);

  return (
    <>
      <div className="aspect-video h-full max-h-[512px] w-full gap-5 flex">
        <div
